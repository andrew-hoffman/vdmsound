Building this project requires a 32-bit Windows environment with Visual C++ 6.0 Enterprise Edition Service Pack 5, and the Windows 2000 DDK (Driver Development Kit). You will likely want to set up a Windows 2000 or XP virtual machine for this purpose. 
...
- `VDMSCorePath` pointing to the VDMSCore directory inside the source tree
- `NTDDKPath` pointing to the NTDDK install location (default C:\NTDDK or C:\WinDDK)
- `BCPATH` path to Borland C++ 5.5 16-bit compiler

Building is done from inside Visual Studio, open the .dsw Workspace file inside the VDMSCore or VDMSModules folder and select your build targets with Build -> Batch Build...
...
Clean builds of VDMSCore seem to crash the WinHelp compiler at least in VMWare but after a restart the build will work
